...
- Removed `/repairs`' attempt to fetch HA Core's own issue registry via `core/api/repairs/issues` — that REST endpoint has never existed (the issue registry is websocket-only), so it failed with 401 on every call and logged a warning. `/repairs` now only reports Supervisor resolution-center issues, which is the data it can actually fetch, restoring the `description` field the merge had dropped.
